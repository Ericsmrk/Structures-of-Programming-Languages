...
-- t = (Node2 20 (Node2 15 (Leaf 7) (Leaf 17)) (Node2 30 (Node2 23 (Leaf 21) (Leaf 25)) (Leaf 34)))
-- t2 =
t3 = (Node2 3 (Leaf 1) (Leaf 4))
...
help2 :: ExtInt -> ExtInt -> Tree2 Int -> Bool
help2 x y (Leaf n) = if (((Fin n) <= x) || ((Fin n) >= y)) then False else True
help2 x y (Node2 n l r ) = if (((Fin n) <= x) || ((Fin n) >= y))
                         then False
                         else (help2 x (Fin n) l) && (help2 (Fin n) y r)

bst2 :: Tree2 Int -> Bool
bst2 (Leaf n) = False
bst2 (Node2 n l r) = help2 NegInf PosInf (Node2 n l r)
